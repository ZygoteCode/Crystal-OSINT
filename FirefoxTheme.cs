...
static class Helpers
{
    ...
    public static void CenterString(Graphics G, string Text, Font Font, Color C, RectangleF Rect)
    {
...
class FirefoxSubTabControl : TabControl
{
    ...
    private Color mainColor = Color.FromArgb(12, 217, 255);
    private Timer animationTimer;
    private int animationProgress = 0;
    private int oldSelectedIndex = -1;
    #region " Control "

    public FirefoxSubTabControl() : base()
    {
        ...
        this.Padding = new Point(14, 5);
        this.ItemSize = new Size(160, 48);
    ...
    {
        return index == this.SelectedIndex ? mainColor : Color.FromArgb(44, 44, 44);
    }

    protected override void OnPaint(PaintEventArgs e)
    {
        base.OnPaint(e);
        e.Graphics.Clear(Color.FromArgb(20, 20, 20)); // Sfondo scuro e futuristico
        ...
        for (int i = 0; i < TabPages.Count; i++)
        {
            ...
            {
                // Calcola il colore della tab selezionata con l'animazione di transizione
                Color oldColor = oldSelectedIndex == -1 ? Color.FromArgb(44, 44, 44) : GetTabColor(oldSelectedIndex);
                tabBackgroundColor = InterpolateColor(oldColor, mainColor, animationProgress / 100.0);
                tabTextColor = Color.White; // Testo bianco per la tab attiva
            }
            else
            {
                // Tab inattive
                tabBackgroundColor = Color.FromArgb(44, 44, 44);
                tabTextColor = Color.FromArgb(163, 163, 163); // Testo grigio per le tab inattive
            }
            ...
            Font tabFont = new Font("Montserrat", 10F, i == this.SelectedIndex ? FontStyle.Bold : FontStyle.Regular);
            Helpers.CenterString(e.Graphics, CurrentTab.Text, tabFont, tabTextColor, TabRect);

            // Evidenziazione della tab selezionata
            if (i == this.SelectedIndex)
            {
                // Animazione di un bordo inferiore luminoso
                using (Pen p = new Pen(InterpolateColor(Color.FromArgb(44, 44, 44), mainColor, animationProgress / 100.0), 3))
                {
                    e.Graphics.DrawLine(p, TabRect.Left, TabRect.Bottom, TabRect.Right, TabRect.Bottom);
                }
            }
        }
    }
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        for (int i = 0; i <= TabPages.Count - 1; i++)
        {
            ...
            if (SelectedIndex == i)
            {
                ...
                using (SolidBrush B = new SolidBrush(Color.FromArgb(12, 217, 255)))
                {
            ...
            if (i == 0)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.database_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 1)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.male_user_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 2)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.Email_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 3)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.website_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 4)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.web_address_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 5)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.phone_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 6)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.discord_new_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 7)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.hashtag_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
            else if (i == 8)
            {
                G.DrawImage(Crystal_OSINT.Properties.Resources.name_50px, new Rectangle(TabRect.X + 19, TabRect.Y + ((TabRect.Height / 2) - 10), 22, 22));
            }
        }
